...
#JAJ    $HOMEgfs/ush/getdump.sh $CDATE $CDUMP $DMPDIR/${CDATE}/${CDUMP}${DUMP_SUFFIX} $COMOUT
...
#JAJ     $HOMEgfs/ush/getdump.sh $GDATE $GDUMP $DMPDIR/${GDATE}/${GDUMP}${DUMP_SUFFIX} $gCOMOUT
...
#JAJ    [[ $ROTDIR_DUMP = "NO" ]] && cp $DMPDIR/$CDATE/$CDUMP/${CDUMP}.t${cyc}z.syndata.tcvitals.tm00 $COMOUT/
...
#JAJ          COMIN_OBS=${COMIN_OBS:-$DMPDIR/$CDATE/$CDUMP}
        ...
        status=$?
        [[ $status -ne 0 ]] && exit $status
    elif [ $machine = "Discover" -o  $machine = "S4" ]; then
...
#JAJ          COMIN_OBS=${COMIN_OBS:-$DMPDIR/$CDATE/$CDUMP}
    ...
    if [ $ROTDIR_DUMP = "NO" ]; then
#JAJ	$NCP $DMPDIR/$CDATE/$CDUMP/${OPREFIX}prepbufr               $COMOUT/${OPREFIX}prepbufr
#JAJ	$NCP $DMPDIR/$CDATE/$CDUMP/${OPREFIX}prepbufr.acft_profiles $COMOUT/${OPREFIX}prepbufr.acft_profiles
#JAJ	[[ $DONST = "YES" ]] && $NCP $DMPDIR/$CDATE/$CDUMP/${OPREFIX}nsstbufr $COMOUT/${OPREFIX}nsstbufr
